...
{
	UDataTable* CommandTable = LoadObject<UDataTable>(nullptr, TEXT("/UniversalModulerEditor/Config/Datatable/MenuConfig"));
	if (!CommandTable) return;

	// 注册时通过引用传递到Map，由Map控制其生命周期
	FSlateStyleSet* MyStyleSet = new FSlateStyleSet("UMEEditor");
