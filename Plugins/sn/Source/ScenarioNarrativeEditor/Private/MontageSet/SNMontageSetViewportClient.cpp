...
FSNMontageSetViewportClient::FSNMontageSetViewportClient(const TSharedRef<SEditorViewport>& InThumbnailViewport,
    ...
    : FEditorViewportClient(nullptr, &InPreviewScene.Get(), StaticCastSharedRef<SEditorViewport>(InThumbnailViewport)),
      ViewportWidget(InThumbnailViewport)
{
    PreviewScene = StaticCastSharedPtr<FSNMontageSetPreviewScene>(InPreviewScene.ToSharedPtr());
    SetRealtime(true);
...
// copy from https://github.com/Sythenz/AssetEditorTemplate/blob/main/Source/AssetEditorTemplateEditor/Private/AssetEditor/SimpleAssetViewport.cpp
void FSNMontageSetViewportClient::FocusViewportOnBounds(const FBoxSphereBounds& Bounds, bool bInstant)
{
