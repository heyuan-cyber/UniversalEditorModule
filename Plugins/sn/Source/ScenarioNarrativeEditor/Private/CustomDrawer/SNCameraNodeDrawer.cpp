...
void FSNCameraNodeDrawer::OnSlateRepaint(const USNAssetBase* Asset, const TSharedPtr<SOverlay>& Slot, const FSNSequencerContext* Context)
{
    const USNCameraNode* Node = Cast<USNCameraNode>(Asset);
                    ...
                    [
                        ...
                        [
                            ...
                            .OnMouseButtonDown_Lambda([Context, CurrentIndex, Asset, DragWidget, this](const FGeometry& Geometry, const FPointerEvent& MouseEvent) -> FReply
                            {
                                ...
                                {
                                    USNCameraNode* Node = Cast<USNCameraNode>(const_cast<USNAssetBase*>(Asset));
                                    USNCameraKeyFrameWrapper* Wrapper = CreateTransientObject<USNCameraKeyFrameWrapper>();
                                    Wrapper->CameraKeyFrame = Node->KeyFrames[CurrentIndex];
                            ...
                            [
                                ...
                                .ColorAndOpacity_Lambda([this, Context, CurrentIndex]() -> FSlateColor {
                                    ...
                                    if (object != nullptr && IsValid(object)) // 假设 UObjectA 有一个 bool 属性 bIsActive
                                    {
                                        USNCameraKeyFrameWrapper* CameraKeyFrameWrapper = Cast<USNCameraKeyFrameWrapper>(object);
                                        if(CameraKeyFrameWrapper)
                                            return CameraKeyFrameWrapper->KeyFrameId == CurrentIndex ? FLinearColor::Red : FLinearColor::White; // 激活时为绿色
                                    }
...
int FSNCameraNodeDrawer::OnWidgetPaint(const USNAssetBase* Asset, const FSNSequencerContext* Context, const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
    if (!Asset->Validate())
        return LayerId;
